...
export default function DrawingActivity({ socket, roomId, role }) {
  ...
  return (
    <div className="bg-white p-4 rounded shadow-md mb-4">
      <h2 className="text-xl font-semibold mb-2">Çizim Araçları</h2>
      <div className="flex gap-4 mb-4">
        ...
        <div>
          <label><input type="checkbox" checked={eraser} onChange={(e) => setEraser(e.target.checked)} /> Silgi</label>
        </div>
        <button onClick={handleCombine} className="bg-coupleAccent hover:bg-pink-600 text-white py-1 px-3 rounded">Tamamla ve İndir</button>
      </div>

      <div className="flex gap-4">
        <div>
          <h2 className="text-xl font-semibold mb-2">Partner Kanvası (Sol)</h2>
          <canvas
        ...
        <div>
          <h2 className="text-xl font-semibold mb-2">Kullanıcı Kanvası (Sağ)</h2>
          <canvas
            ref={userCanvasRef}
